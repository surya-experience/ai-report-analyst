...
Preview (`src/components/reports/preview-dialog.tsx`) renders as a modal with switchable Bar/Donut/Pie views for single-metric reports, or — for the three "one row per thing over time" reports (the two campaign reports and Profile Statistics) — pages through individual items with prev/next arrows, stat tiles, and a real day-by-day trend graph.
